...
batcov <- batcov[ ,c("host_species", "betacov", "sarbecov")]
batcov <- distinct(batcov)
...
make.map.data <- function(data, n.boots, simp.list){
  ...
  for(i in 1:n.boots){
    ...
    # glm.fit warning messages due to small sample size--can ignore
     gbm.reduced <- gbm.step(data = boot.dat,
    ...
    test.dat = rbind.fill(test.pres.data, test.abs.data)

    ## Predict and assess model
    test.dat$preds <- predict.gbm(gbm.reduced,
                         test.dat,
...
write.csv(pred_merge, "GuthPredictions.csv")
